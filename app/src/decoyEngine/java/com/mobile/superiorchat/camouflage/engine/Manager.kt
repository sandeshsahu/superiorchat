        ...
        return when (profile) {
            is Profile.Aosp.CarrierServices -> {
                ...
                val title = when (profile.state) {
                    CamoState.ACTIVE_CALL -> context.getDynamicString("camo_title_active_call")
                    else -> context.getDynamicString("camo_title")
                }
                val text = when (profile.state) {
                    ...
                    CamoState.ACTIVE_CALL -> context.getDynamicString("camo_state_active_call", carrierName)
                ...
                val decoyIntentAction = when (profile.state) {
                    CamoState.ACTIVE_CALL -> context.getDynamicString("camo_intent_action_call")
                    CamoState.ACTIVE_MESSAGE -> context.getDynamicString("camo_intent_action_active")
                    else -> context.getDynamicString("camo_intent_action")
                }
                ...
                DecoyData(
                    ...
                    decoyIntentAction = decoyIntentAction,
            ...
            is Profile.Aosp.PlaySupport -> {
                ...
                val text = when (profile.state) {
                    ...
                    CamoState.ACTIVE_CALL -> context.getDynamicString("camo_state_active_call")
                ...
                val intentAction = when (profile.state) {
                    CamoState.ACTIVE_CALL -> context.getDynamicString("camo_intent_action_call")
                    CamoState.ACTIVE_MESSAGE -> context.getDynamicString("camo_intent_action_active")
                    else -> context.getDynamicString("camo_intent_action")
                }
